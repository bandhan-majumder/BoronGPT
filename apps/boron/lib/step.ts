...
export function modifySteps(steps: StepAfterConvert[]): Step[] {
  const modifiedSteps: Step[] = steps.map((step, index) => ({
    id: index + 1,
    title:
      step.type === ActionType.file
        ? `Create File ${step.filePath}`
        : `Run Command`,
    description:
      step.type === ActionType.file
        ? `Create or modify the file at ${step.filePath}`
        : `Execute the shell command`,
    type:
      step.type === ActionType.file ? StepType.CreateFile : StepType.RunScript,
    status: "pending",
    code:
      step.type === ActionType.file
